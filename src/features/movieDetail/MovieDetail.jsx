...
class MovieDetail extends Component {
  render() {
    const imgMovie = '/assets/images/' + detail.title.toLowerCase().replace(/\s/g,'') + '.jpg';
    return (
      <Row className="my-5">
        <Col xs="12" md="4">
          <div>
          ...
          <img className="img-fluid" src={imgMovie} alt="movie img" />
        </Col>
        <Col xs="12" md="8">
          <div>{detail.director}</div>
          <h1>{detail.title}</h1>
          <div className="list-inline mb-3">
            <div className="list-inline-item">Producer: {detail.producer}</div>
            <div className="list-inline-item">Release Date: {detail.release_date}</div>
          </div>
